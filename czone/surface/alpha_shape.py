...
def tetrahedron_circumradii(points):
    ...
    points = np.array(
        points
    )  # + np.random.rand(points.shape[0], points.shape[1], points.shape[2])*1e-8 # add small random number to avoid divide by zero
...
def alpha_shape_alg_3D(points, probe_radius, return_alpha_shape=False):
    ...
    circumradii = tetrahedron_circumradii(
        (points + 1e-10 * np.random.rand(*points.shape))[tri.simplices, :]
    )
    ...
    out_points = sub_points[
        np.logical_not(
            np.logical_or(sub_neighbors == -1, np.logical_not(probe_test[sub_neighbors]))
        )
    ]
...
def alpha_shape_alg_3D_with_sampling(
    points, probe_radius, N_samples, std=1e-4, return_alpha_shape=False, rng=None
):
    """Use alpha shape algorithm to determine points on exterior of collection of points.
